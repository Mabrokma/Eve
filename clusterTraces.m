function [whichCluster, nc14, time14] = clusterTraces(genotype)
%**************************************************************************
% (Under development)
%
...
axis([0 12000 0 750])
title(['Histogram of (nonzero) fluorescence intensities: ', genotype.Name])

%Cluster on the logical on/off version of the fluo vector
...
for k = 2:8
    ...
    subplot(2,2,1)
    whichCluster{k} = kmeans(fluonc14, k, 'Replicates', 5);
    [~, order] = sort(whichCluster{k});
    ...
    for i = 1:k
        hold on
        %Label each cluster
        text(mean(find(sort(whichCluster{k})==i)), 5, num2str(i),...
            'Color', [1 1 1], 'FontWeight', 'bold', 'EdgeColor', [1 1 1]);
        
        %Plot lines between clusters
        if i ~= k
            plot(find(sort(whichCluster{k})==i, 1, 'last')*ones(1,2),...
                [0 100], 'm', 'Linewidth', 2)
        end
        ax = gca;
        ax.XTick = [];
    ...
    %Plot mean traces for each cluster & histogram clusters by AP Position
    leg = cell(1,k);
    for i = 1:k
        subplot(2,2,2)
        hold on
        plot(time14, nanmean(fluonc14(whichCluster{k} == i, :)),...
        ...
        clusterHist = histcounts(...
            nanmean(nc14(:,whichCluster{k}==i,3)), 0.2:0.01:0.85);
        hold on
